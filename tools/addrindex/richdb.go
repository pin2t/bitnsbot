...
func openRich(path string) (*richStore, error) {
    ...
    for _, p := range richPragmas {
        if _, err := db.Exec(p); err != nil {
            db.Close()
            return nil, fmt.Errorf("%s: %w", p, err)
        }
    }
    for _, ddl := range []string{richDDL, balancesDDL("balances"), metaDDL} {
        if err := ensure(db, ddl); err != nil {
            db.Close()
            return nil, fmt.Errorf("%s: %w", ddl, err)
        }
    }
    return &richStore{db: db}, nil
}

func (s *richStore) close() error { return s.db.Close() }
...
func (s *richStore) meta(key string) (string, bool, error) {
    var text string
...
func (s *richStore) height(key string) (int, bool, error) {
