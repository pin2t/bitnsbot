// Command bboltwui is a web UI for a bbolt database: point it at any bbolt file
// and browse, edit, delete, export or import its buckets from a browser. The
// whole UI is this file and the one page it embeds, so the tool needs nothing
// from the bot. See the README beside this file.
package main
...
func main() {
    ...
    var err error
    if db, err = bbolt.Open(*dbPath, 0600, nil); err != nil {
        logging.Fatal("open %s: %v", *dbPath, err)
    }
    var srv = &http.Server{Addr: *listenAddr, Handler: handler()}
    go func() {
        logging.Status("database UI listening on %s", *listenAddr)
...
// db is the database main opened. It is a package global rather than a
// parameter on every handler because there is exactly one of it, so each route
// registers its handler by name instead of through a closure.
var db *bbolt.DB
