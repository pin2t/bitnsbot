...
func TestInfoFlow(t *testing.T) {
	...
	handleUpdate(bot, Update{Message: &Message{Chat: Chat{ID: 1}, Text: "/info"}})
	...
	handleUpdate(bot, Update{Message: &Message{Chat: Chat{ID: 1}, Text: "hello world"}})
	...
	handleUpdate(bot, Update{Message: &Message{Chat: Chat{ID: 2}, Text: "/info direct arg"}})
	if len(sent) != 3 || sent[2] != "Info: direct arg" {
		t.Fatalf("unexpected third reply: %#v", sent)
	}
	handleUpdate(bot, Update{Message: &Message{Chat: Chat{ID: 3}, Text: "/start"}})
	...
	handleUpdate(bot, Update{Message: &Message{Chat: Chat{ID: 4}, Text: "just chatting, no pending info"}})
	if len(sent) != 4 {
