...
func TestUnconfirmedTransaction(t *testing.T) {
    ...
    defer server.Close()
    var bot = newBot("TESTTOKEN", server.URL)
    var mainTxid = "aaaa47a9143a23e80cc59e81588d21558b394005580b285961957cb3bed5b3e0"
    var seen = time.Now().Add(-5 * time.Minute).Unix()
    var btcdServer = newFakeBtcdServer(t, func(method string, params json.RawMessage) (interface{}, error) {
        var p []interface{}
        json.Unmarshal(params, &p)
        switch method {
        case "getrawtransaction":
            if id, _ := p[0].(string); id == "prevtx" {
                return map[string]any{"vout": []map[string]any{{"value": 0.30010113, "scriptPubKey": map[string]any{"address": "bc1qinput"}}}}, nil
            }
            return map[string]any{"txid": mainTxid, "confirmations": 0, "size": 193,
                "vin":  []map[string]any{{"txid": "prevtx", "vout": 0}},
                "vout": []map[string]any{{"value": 0.3, "scriptPubKey": map[string]any{"address": "bc1qoutput"}}}}, nil
        case "getrawmempool":
            return map[string]any{mainTxid: map[string]any{"time": seen}}, nil
        }
        return nil, fmt.Errorf("unexpected method %s", method)
    })
    defer btcdServer.Close()
    btcd = dialFakeBtcd(t, btcdServer, &recordingHandler{})
    defer func() { btcd.close(); btcd = nil }()
    update(bot, Update{Message: &Message{Chat: Chat{ID: 1}, Text: "/info " + mainTxid}})
    if len(sent) != 1 {
        t.Fatalf("expected one reply, got %#v", sent)
    }
    for _, want := range []string{
        "unconfirmed (in mempool)", "Seen:", "5 minutes ago", "Amount:", "30 000 000 satoshi",
        "Fee:", "10 113 satoshi", "Size:", "193 bytes", "Inputs:", "bc1qinput", "Outputs:", "bc1qoutput",
    } {
        if !strings.Contains(sent[0], want) {
            t.Fatalf("unconfirmed tx reply missing %q: %q", want, sent[0])
        }
    }
}
