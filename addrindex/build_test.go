...
// interface (/rest/block/<hash>.bin), with the per-transaction output and
// spent-prevout scripts Core itself reported via getblock verbosity 3 for the
// same blocks. block102 has one coinbase and one spend; block103 has a coinbase
// and two further spends chained off it. Parsing has to agree with the node
// exactly, since a mismatch means silently missing or fabricating address
// history.
...
// testBlock is a fixture as a Block: its outputs parsed from the serialized
// block, and its spends as the scripts the node reported.
func testBlock(t *testing.T, name string) Block {
    for _, c := range parseBlockCases() {
        if c.name != name { continue }
        var raw, _ = hex.DecodeString(c.block)
        var outputs, ok = parseBlockOutputs(raw)
        if !ok { t.Fatalf("%s: parseBlockOutputs failed", name) }
        var blk = Block{Hash: name}
        for i := range outputs {
            var tx = Tx{Outputs: outputs[i]}
            for _, script := range c.wantSpent[i] {
                var b, _ = hex.DecodeString(script)
                tx.Spent = append(tx.Spent, Payment{Script: b})
            }
