...
`isTxid` (shared with `watch()`) and `when`/`ago`/`short` (shared across the per-kind formatters) are real multi-call-site helpers. The per-kind functions themselves are single-call-site — they exist because `info()` had grown too big and splitting it was explicitly requested, not as license to extract small single-use helpers.
